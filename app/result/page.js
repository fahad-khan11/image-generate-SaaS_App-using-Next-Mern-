  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col min-h-[90vh] justify-center items-center'>
      ...
      <div className='relative'>
        <Image src={image} alt="" className='max-w-sm rounded'/>
        <span className={ `absolute bottom-0 left-0 h-1 ${loading?'bg-blue-500 w-full transition-all duration-[10s]':'w-'}`}/> 
      </div>
...
{isImageLoaded &&(
    <div className='flex gap-2 flex-wrap justify-center text-white text-sm p-0.5 mt-10 rounded-full'>
      ...
      className='bg-transparent border border-zinc-900 px-10 py-3 text-black rounded-full cursor-pointer'>Generate Another</p>
